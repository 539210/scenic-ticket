...
后台分类支持修改父级并阻止循环；景点支持名称、分类、价格、状态、简介、图片地址和 JSON 扩展属性维护。用户侧的概览、简介和评论使用独立页签，历史字符串 `item_id` 详情/评论可兼容读取并在编辑时规范化。评论提交由服务层重查已支付未退款订单，同一用户对同一景点再次提交会修改原评论；列表显示脱敏用户、评分、正文和创建/更新时间。
