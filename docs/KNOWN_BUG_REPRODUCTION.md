...
修复结果：评论改由 `CommentService` 和专用 DTO 读取，兼容数值/字符串 `user_id`、`item_id`；独立评论页签显示脱敏用户名、评分、正文、标签、创建时间和更新时间。真实隔离库测试已证明支付资格、首次发布、再次原位更新、UTF-8 中文往返、唯一记录及评分聚合。历史已变成问号的正文不可逆，系统不伪造恢复内容，仍以明确降级提示展示。
