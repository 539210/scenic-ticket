...
- `admissions` 表及其外键关系继续保留，以兼容历史核销记录和“已核销订单不可退款”的业务规则；2026-07-15 起，后台界面不再提供核销操作入口。
- 月度订单报表仍以 `sp_monthly_order_report` 生成按日汇总；管理员点击某日汇总行后，会通过订单查询展示同日已支付/已完成订单明细，明细中的用户、景点、票种、数量与金额均以 `orders` 中的快照字段为准。
- 景点创建服务会在写入 `items` 后自动创建一张同价成人票和未来 31 天每日库存；该规则是应用服务层的创建策略，不改变本 E-R 图中的表关系。
- 所有库存写入继续以 `ticket_inventory(ticket_type_id, visit_date)` 唯一键和行锁为边界，不依赖 Swing 界面计算可售数量。
