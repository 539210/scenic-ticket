...
public final class CommentDialogPanel extends JPanel {
    ...
    public CommentSubmission submission() {
        return new CommentSubmission(commentArea.getText(), (Integer) ratingSpinner.getValue());
    }

    void setFormValues(String content, int rating) {
        commentArea.setText(content);
        ratingSpinner.setValue(rating);
    }

    public record CommentSubmission(String content, int rating) {
    }
}
